...
因中壢測站(C0C700)年報表缺少資訊較多，因此改成使用桃園測站(C0C480)進行繪製。
### code : 
...
start_date=2011;
end_date=2020;
Target_Weathers.OneYear.DataHeader={'年累積降水量(mm)'};
...
for i_datenumber=start_date:end_date
    ...
    if (sum(isnan(temp_data2)) >= 10)
        Target_Weathers.OneYear.Data(i_datenumber-datenum(start_date)+1,3)=NaN;
    elseif (length(temp_data2) == 12)
            temp_data2(isnan(temp_data2))=[];
            if ~isempty(temp_data2)
                Target_Weathers.OneYear.Data(i_datenumber-datenum(start_date)+1,3)=sum(temp_data2);
            else
                Target_Weathers.OneYear.Data(i_datenumber-datenum(start_date)+1,3)=NaN;
            end
    else
    ...
    OneYear_rainfall=Target_Weathers.OneYear.Data(:,3)
...
bar(Target_year,OneYear_rainfall); % 長條圖繪製每年雨量
